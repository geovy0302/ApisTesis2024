...
FROM  citasmedicas a INNER JOIN centros_de_atencio b ON  (b.Id_CentroHosp = a.Id_CentroAteFK)  INNER JOIN lista_de_medicos c ON (c.Id_medico = a.Id_MedicoFK) INNER JOIN especialidades_medi d ON (d.Id_Especialidad = c.Id_EspecialidadFK) INNER JOIN usuariospacientes e ON (e.Id_Usuario = a.Id_UsuarioFK) && (e.Id_Usuario = ?);");
...
if ($querysAmarre->rowCount() > 0) {
    ...
    if($response != null){
        ...
        $stuff["message"] = "Listados de Citas Médicas encontrados";
        array_push($response, $stuff);
